...
- Media activation, rendering, and seeking reject stale generations at the worker boundary. Composition snapshots use a separate revision so they remain reusable across normal clip changes and are discarded when an edit replaces the composition. The controller continues to discard stale worker events and frames before they can update the session or preview.
