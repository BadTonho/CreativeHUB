...
| Timeline model | 24/30/60 FPS sources on a 30 FPS Timeline; Timeline/source duration separation; mixed-rate split, trim, nonzero source in-point, source bounds, tracks, ordering, gaps, overlap rules, movement, rolling and individual edge trims, one-sided media overlap and top-clip priority, still-image/text extension, delete, metadata, canonical multi-track snapshots, history, Undo, and Redo |
...
| Project persistence | Version 11 rational Timeline-rate and source-duration round-trip; invalid rational rates; persisted stable track/clip IDs; canonical multi-track video/image/text kind round-trip; media overlap round-trip with text-over-text rejection; timeline zoom and row-height persistence; version 1–10 migration with legacy flat clips converted to `timeline_tracks`; duplicate/zero ID rejection; invalid input; offline media; transactional open |
...
- with Preview metrics enabled, play a layered Timeline containing an opaque,
  ...
  forward-decoded video layers, check the decoder start and requested frames,
